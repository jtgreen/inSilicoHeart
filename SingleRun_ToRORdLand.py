...
def modelRunner( X0, parameters, beats, ignoreFirst):
    ...
    stimAmp = -53
    if 'stimAmp' in parameters: stimAmp = parameters['stimAmp']
    stimDur = 1
    if 'stimDur' in parameters: stimDur = parameters['stimDur']
    ...
    #for n=1:beats
    for n in range(0,beats): # JTG: Change this to 0 indexing format
        if verbose:
            print('Beat = ', str(n))
...
def model_ToRORd_Land(X,t,flag_ode, cellType, ICaL_Multiplier, INa_Multiplier, Ito_Multiplier, INaL_Multiplier, 
    ...
    # gamma_rate=gamma*max((ZETAS>0).*ZETAS,(ZETAS<-1).*(-ZETAS-1)) # JTG: They are arrays?
    gamma_rate=gamma*max((ZETAS>0)*ZETAS,(ZETAS<-1)*(-ZETAS-1))
    xb_su_gamma=gamma_rate*XS
    gamma_rate_w=gamma_wu*np.abs(ZETAW) # % weak xbs don't like being strained
    ...
    # https://stackoverflow.com/questions/39885495/what-is-the-meaning-of-single-quote-in-matlab-and-how-to-change-it-to-python#:~:text=It%20flips%20the%20matrix%20around,a'%20in%20Python%20is%20np.
    if flag_ode==1:

        # np.atleast_2d(a).T.conj()
        ...
        # return np.atleast_2d([dv, dnai, dnass, dki, dkss, dcai, dcass, dcansr, dcajsr, dm, dhp, dh, dj, djp, dmL, dhL, dhLp, da, diF, diS, dap, diFp, diSp,
        #     dd, dff, dfs, dfcaf, dfcas, djca, dnca, dnca_i, dffp, dfcafp, dxs1, dxs2, dJrel_np, dCaMKt,
...
def getINa_Grandi(v, m, h, hp, j, jp, fINap, ENa, INa_Multiplier):
    ...
    bh = (v >= -40) * (0.77 / (0.13*(1 + np.exp( -(v + 10.66) / 11.1 )))) + (v < -40) * ((2.7 * np.exp( 0.079 * v) + 3.1*10**5 * np.exp(0.3485 * v)))
    tauh = 1 / (ah + bh)
    hss = 1 / ((1 + np.exp( (v + 71.55)/7.43 ))**2)
    dh = (hss - h) / tauh
    
    # %% j gate
    aj = (v >= -40) * (0) +(v < -40) * (((-2.5428 * 10**4*np.exp(0.2444*v) - 6.948*10**-6 * np.exp(-0.04391*v)) * (v + 37.78)) / (1 + np.exp( 0.311 * (v + 79.23) )))
    bj = (v >= -40) * ((0.6 * np.exp( 0.057 * v)) / (1 + np.exp( -0.1 * (v + 32) ))) + (v < -40) * ((0.02424 * np.exp( -0.01052 * v )) / (1 + np.exp( -0.1378 * (v + 40.14) )))
    tauj = 1 / (aj + bj)
...
def getICaL_ORd2011_jt(v, d,ff,fs,fcaf,fcas,jca,nca, nca_i, ffp, fcafp, fICaLp, cai, cass, cao, nai, nass, nao, ki, kss,ko, cli, clo, celltype, ICaL_fractionSS, ICaL_PCaMultiplier):
...
def getIKr_ORd2011_MM(V,c0,c1, c2, o, i, ko, EK, celltype, IKr_Multiplier):
    ...
    GKr = 0.0321 * np.sqrt(ko/5) * IKr_Multiplier # % 1st element compensates for change to ko (sqrt(5/5.4)* 0.0362)
    if celltype==1:
        GKr=GKr*1.3
    elif celltype==2:
        GKr=GKr*0.8
...
def getJrel_ORd2011(Jrelnp, Jrelp, ICaL, cass, cajsr, fJrelp, celltype, Jrel_Multiplier):
    jsrMidpoint = 1.7
...
# for each_beat in X:
# 	y.extend([_[0] for _ in each_beat])

y = [each_measurement[0] for each_measurement in X[4]]
# # plt.ylim((0,45))
...
# y = [each_measurement[0] for each_beat in X for each_measurement in each_beat]

# plt.plot(np.linspace(0, 10000, 10000*beats), y)
# # axes=plt.gca()
# # axes.set_aspect(0.1)
# # plt.show()
...
# scale_factor_x = 1
# scale_factor_y = 1
# plt.xlim(xmin * scale_factor_x, xmax * scale_factor_y)
# plt.ylim(ymin * scale_factor_x, ymax * scale_factor_y)
